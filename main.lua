                ...
                if player.leaderstats.Strength.Value >= rebirthCost then
                    ...
                    if currentRebirths >= targetRebirth then
                        ...
                        Library:Notify{
                            ...
                            Content = "Reached target rebirth count! 🎯",
                            Duration = 5
                        }
                        return
                    end
...
Tabs.Killer:AddToggle("Kill Nearby", {
			...
			while getgenv().killNearby do
				pcall(function()
					...
					if myRoot then
						...
						radiusVisual.Size = Vector3.new(150, 0.1, 150)
...
local function cleanAndGetLists()
	...
	for _, petName in ipairs(petsToRemove) do
		local pet = petShopFolder:FindFirstChild(petName)
		if pet then
			pet:Destroy()
		end
	end
	...
	for _, item in pairs(petShopFolder:GetChildren()) do
		if item:FindFirstChild("isPowerUp") then
			table.insert(auraList, item.Name)
		else
			table.insert(petList, item.Name)
		end
	end
	return petList, auraList
end

local petList, auraList = cleanAndGetLists()

local selectedPet = ""
local PetDropdown = Tabs.Crystals:CreateDropdown("PetList", {
	Title = "Select Pet",
	Description = "Choose a pet to auto open",
	Values = petList,
	Multi = false,
	Default = 1,
	Callback = function(Value)
...
local selectedAura = ""
local AuraDropdown = Tabs.Crystals:CreateDropdown("AuraList", {
	Title = "Select Aura",
	Description = "Choose an aura to auto open",
	Values = auraList,
	Multi = false,
	Default = 1,
	Callback = function(Value)
...
Tabs.Crystals:CreateButton({
	Title = "Equip All Tribal Overlord",
	Callback = function()
		for _, pet in pairs(game:GetService("Players").LocalPlayer.petsFolder.Unique:GetChildren()) do
...
Tabs.Crystals:CreateButton({
	Title = "Equip All Mighty Monster",
	Callback = function()
		for _, pet in pairs(game:GetService("Players").LocalPlayer.petsFolder.Unique:GetChildren()) do
